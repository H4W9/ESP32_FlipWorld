...
#else
// PicoDVI is RP2040-only. On the ESP32-C5 Pancake the board always uses the
// TFT_eSPI backend (libraryType == LIBRARY_TYPE_TFT), so every DVI branch in
// draw.cpp is dead code. Provide no-op stubs so those branches still compile.
static const int DVI_RES_320x240p60 = 0;
static int picodvi_dvi_cfg = 0;
struct _PW_DVIStub
{
    template <class... A> _PW_DVIStub(A...) {}
    template <class... A> bool begin(A...) { return true; }
    template <class... A> uint16_t color565(A...) { return 0; }
    template <class... A> void drawBitmap(A...) {}
    template <class... A> void drawCircle(A...) {}
    template <class... A> void drawGrayscaleBitmap(A...) {}
    template <class... A> void drawLine(A...) {}
    template <class... A> void drawPixel(A...) {}
    template <class... A> void drawRGBBitmap(A...) {}
    ...
    uint16_t *getPalette() { return nullptr; }
    ...
    template <class... A> void swap(A...) {}
};
typedef _PW_DVIStub DVIGFX8;
typedef _PW_DVIStub DVIGFX16;
#endif
...
namespace Picoware
{
        ...
        {
        public:
                DisplayAdapter(const Board board, bool use_8bit = true, bool tftDoubleBuffer = true); // Constructor for the DisplayAdapter class. Initializes the display based on the board type and whether to use 8-bit or 16-bit color depth.
                ~DisplayAdapter();
                bool begin();                                                                                                             // Initializes the display.
                uint16_t color565(uint8_t r, uint8_t g, uint8_t b);                                                                       // Converts RGB values to 16-bit color format.
                ...
                TFT_eSPI *getTFT() noexcept { return displayTFT; }                                                                        // Raw panel (TFT backend only; nullptr on DVI).
                TFT_eSprite *getCanvas() noexcept { return canvasTFT; }                                                                   // Off-screen sprite buffer (may be nullptr).
        private:
        ...
        {
        public:
                Draw(const Board board, bool use_8bit = false, bool tftDoubleBuffer = false);
                ~Draw();
                ...
                void fillScreen(uint16_t color);                                                                                                                       // Fills the entire screen with the specified color.
                Board getBoard() const noexcept { return picoBoard; }                                                                                                  // Returns the board configuration.
                Vector getCursor();                                                                                                                                    // Returns the current cursor position.
                uint16_t *getPalette();                                                                                                                                // Returns the color palette.
                Vector getSize() const noexcept { return size; }                                                                                                       // Returns the size of the display.
                bool is8bit() const noexcept { return is_8bit; }                                                                                                       // Returns true if the display is 8-bit, false otherwise.
                bool isDoubleBuffered() const noexcept { return is_DoubleBuffered; }                                                                                   // Returns true if double buffering is used, false otherwise.
                ...
                void imagePGM(Vector position, const uint8_t *bitmapPGM, Vector size, const uint16_t *palette = nullptr, bool imageCheck = true, bool invert = false); // Draws a bitmap stored in program memory on the display at the specified position.
                // FlipWorld colour port: blit a 1-byte-per-pixel PROGMEM mask sprite. Bytes
                // equal to transparentValue are skipped (see-through); every other byte is
                // drawn in inkColor. This replaces the monochrome basicPalette path so the
                // Flipper-style 0x00/0xFF sprites render in full colour on a coloured world.
                void imageMaskPGM(Vector position, const uint8_t *bitmapPGM, Vector size, uint16_t inkColor, uint8_t transparentValue = 0xFF, bool imageCheck = true); // Draws a 1-bit PROGMEM mask sprite in a single colour with transparency.
                void setFont(int font = 2);                                                                                                                            // Sets the font for text rendering.
